...
def recover_merges(mergeable_ranks):
    ...
    # ex: {(220, 220): 256}, recover like our codebase merge dictionary
    merges = {}
    for token, rank in mergeable_ranks.items():
        if len(token) == 1:
            continue  # skip raw bytes, rank from 0 - 255 will be skip
...
class GPT4Tokenizer(RegexTokenizer):
    ...
    def __init__(self):
        ...
        # ex: {b'!': 0, b'"': 1, b'#': 2, b'$': 3, b'%': 4} like vocab in our codebase
        mergeable_ranks = enc._mergeable_ranks
        # the merges are those of gpt4, but we have to recover them
        self.merges = recover_merges(mergeable_ranks)
        # reconstruct the vocab from the merges
        vocab = {idx: bytes([idx]) for idx in range(256)}
        # same as training piece in regex tokenizer, we just need add new vocab idx according to merge file
